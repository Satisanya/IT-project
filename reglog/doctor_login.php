...
if (isset($_POST["submit"])) {
    ...
    $result = mysqli_query($conn, "SELECT * FROM tb_doctor WHERE email = '$email'");
    $row = mysqli_fetch_assoc($result);
    if (mysqli_num_rows($result) > 0) {
        if (password_verify($password, $row['password'])) {
            $_SESSION["login"] = true;
            $_SESSION["flag"] = "is_doctor";
            $_SESSION["doctor_id"] = $row["doctor_id"];
            header("Location: doctor_lk.php");
        } else {
            echo
        ...
        echo
            "<script> alert('Доктор с такими данными не зарегистрирован'); </script>";
    }
}
?>
