...
if (isset($_POST["submit"])) {
  ...
  $result = mysqli_query($conn, "SELECT * FROM tb_sysadmin WHERE email = '$email'");
  $row = mysqli_fetch_assoc($result);
  if (mysqli_num_rows($result) > 0) {
    if (password_verify($password, $row['password'])) {
      $_SESSION["login"] = true;
      $_SESSION["flag"] = "is_sysadmin";
      $_SESSION["sysadmin_id"] = $row["sysadmin_id"];
    ...
    echo
      "<script> alert('Системный администратор с такими данными не зарегистрирован'); </script>";
  }
}
?>
